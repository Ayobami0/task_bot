...
async def payments_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query != None:
        ...
        match query.data:
            case 'not_received':
                await query.message.edit_text(f"{query.message.text}\nPayment not received. Ask customer to check back in {TASK_UPDATE_DURATION}mins. I'll Notify you then.", reply_markup=reply_markup3)
                CountDownExecutor(TASK_UPDATE_DURATION, up(update.get_bot(), message.from_user.name, message_id, chat_id, "I'm notifying you about this task"))
            case 'confirming':
                await query.message.edit_text(f"{query.message.text}\nConfirming Payment...", reply_markup=reply_markup2)
            case 'credited':
        ...
        if message.caption == None:
            await message.reply_text("Add the payment information for the transaction as the image caption")
        else:
            await message.pin()

...
if __version_info__ < (20, 0, 0, "alpha", 1):
    raise RuntimeError(
        f"This example is not compatible with your current PTB version\
{TG_VER}. To view the "
...
from commands.generate_random_pin import (
    generate_random_pin,
    confirm_email,
    cancel,
    CONFIRM_EMAIL,
...
def main() -> None:
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            CONFIRM_EMAIL: [
                MessageHandler(
                    filters.Regex("^\w\w+.@+\w+\.\w+") & filters.TEXT,
                    confirm_email,
                )
            ],
        },
    ...
    application.add_handler(
        CommandHandler("number_of_tasks", tasks_number_command),
    )

    application.add_handler(
        CallbackQueryHandler(
            payments_handler, pattern="confirming|credited|not_received|closed"
        )
    )
    application.add_handler(
        CallbackQueryHandler(verify_user, pattern="yes|no|verified")
    )
    application.add_handler(
        CallbackQueryHandler(get_tasks, pattern="previous_page|next_page")
    )
    ...
    application.add_handler(
        MessageHandler(
            filters.Regex("[vV]+erify") & filters.TEXT & ~filters.COMMAND,
            verify_user,
        )
    )
    application.add_handler(
        MessageHandler(
            filters.PHOTO & PaymentsFilter() & ~filters.COMMAND,
            payments_handler,
        )
    )
